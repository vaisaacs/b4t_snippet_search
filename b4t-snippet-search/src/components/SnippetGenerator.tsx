...
export default function SnippetGenerator({ client, inputs, onInputChange }: SnippetGeneratorProps) {
  ...
  const formatCareNoteSnippetList = (notes: { cleanText: string; tagFound: string }[]): string => {
    if (notes.length === 0) return 'None on file';
    return notes.map(n => `* [${n.tagFound}] ${n.cleanText}`).join('\n');
  };

  // Compile exact 1-row Tab-Separated layout to copy-paste into Google Sheets cells!
  const tabSeparatedSnippet = useMemo(() => {
    const t0Text = parsedCare.t0.map(n => `[${n.tagFound}] ${n.cleanText}`).join('; ') || 'None';
    const t1Text = parsedCare.t1.map(n => `[${n.tagFound}] ${n.cleanText}`).join('; ') || 'None';
    const t2Text = parsedCare.t2.map(n => `[${n.tagFound}] ${n.cleanText}`).join('; ') || 'None';
    const t3Text = parsedCare.t3.map(n => `[${n.tagFound}] ${n.cleanText}`).join('; ') || 'None';
    const t4Text = parsedCare.t4.map(n => `[${n.tagFound}] ${n.cleanText}`).join('; ') || 'None';
    const t5Text = parsedCare.t5.map(n => `[${n.tagFound}] ${n.cleanText}`).join('; ') || 'None';
...
T0 Client Care Notes:
${formatCareNoteSnippetList(parsedCare.t0)}
