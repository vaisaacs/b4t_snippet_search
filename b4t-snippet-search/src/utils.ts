...
export function parseClientCareNotes(rawNotes: string): ClientCareNotesParsed {
  ...
  // Split rawNotes by looking for "\n* " or starting with "* "
  // This ensures that newlines within a note do not create a new note
  const rawParts = rawNotes.split(/(?:^|\n)\s*\*\s+/).filter(Boolean);

  for (let note of rawParts) {
    const cleanNote = note.trim();
    if (!cleanNote) continue;
    
    const lowerNote = cleanNote.toLowerCase();

    let targetList: keyof ClientCareNotesParsed = 't5';
    let tagFound = 'Unknown Tier';
    let description = 'General care note';

    // Match subtypes first, falling back to broader types
    if (/\[?(tier\s*|t)0\]?/i.test(lowerNote)) {
      targetList = 't0'; tagFound = 'T0'; description = 'Client is not upset and requires no reassurance';
    } else if (/\[?(tier\s*|t)1\]?/i.test(lowerNote)) {
      targetList = 't1'; tagFound = 'T1'; description = 'Client needs some reassurance';
    } else if (/\[?(tier\s*|t)2\s*-\s*resolved\]?/i.test(lowerNote)) {
      targetList = 't2'; tagFound = 'T2 - Resolved'; description = 'The tier 2 issue has been resolved';
    } else if (/\[?(tier\s*|t)2\]?/i.test(lowerNote)) {
      targetList = 't2'; tagFound = 'T2'; description = 'Client is very upset';
    } else if (/\[?(tier\s*|t)3a\]?/i.test(lowerNote)) {
      targetList = 't3'; tagFound = 'T3a'; description = 'Client-initiated cancellation, retainer credit in account';
    } else if (/\[?(tier\s*|t)3b\]?/i.test(lowerNote)) {
      targetList = 't3'; tagFound = 'T3b'; description = 'Client-initiated cancellation, retainer exhausted / balance remains';
    } else if (/\[?(tier\s*|t)3c\]?/i.test(lowerNote)) {
      targetList = 't3'; tagFound = 'T3c'; description = 'Attorney-initiated cancellation, retainer credit in account';
    } else if (/\[?(tier\s*|t)3\s*-\s*resolved\]?/i.test(lowerNote)) {
      targetList = 't3'; tagFound = 'T3 - Resolved'; description = 'Successfully saved client from canceling';
    } else if (/\[?(tier\s*|t)3\]?/i.test(lowerNote)) {
      targetList = 't3'; tagFound = 'T3'; description = 'Cancellation of service request';
    } else if (/\[?(tier\s*|t)4\s*-\s*resolved\]?/i.test(lowerNote)) {
      targetList = 't4'; tagFound = 'T4 - Resolved'; description = 'Negative review taken down';
    } else if (/\[?(tier\s*|t)4\]?/i.test(lowerNote)) {
      targetList = 't4'; tagFound = 'T4'; description = 'Negative review posted on social media';
    } else if (/\[?(tier\s*|t)5a\]?/i.test(lowerNote)) {
      targetList = 't5'; tagFound = 'T5a'; description = 'Client-initiated reassignment, retainer credit in account';
    } else if (/\[?(tier\s*|t)5b\]?/i.test(lowerNote)) {
      targetList = 't5'; tagFound = 'T5b'; description = 'Client-initiated reassignment, retainer exhausted / balance remains';
    } else if (/\[?(tier\s*|t)5c\]?/i.test(lowerNote)) {
      targetList = 't5'; tagFound = 'T5c'; description = 'Attorney-initiated reassignment, retainer credit in account';
    } else if (/\[?(tier\s*|t)5\]?/i.test(lowerNote)) {
      targetList = 't5'; tagFound = 'T5'; description = 'General Care Check / Reassignment';
    } else {
      // General fallback if no specific subtype was matched, but it contains a tier mention
      if (/\[?t(ier\s*)?5[^\]]*\]?/i.test(lowerNote)) { targetList = 't5'; tagFound = 'T5'; description = 'General Care Check'; }
      else if (/\[?t(ier\s*)?4[^\]]*\]?/i.test(lowerNote)) { targetList = 't4'; tagFound = 'T4'; description = 'Negative review posted'; }
      else if (/\[?t(ier\s*)?3[^\]]*\]?/i.test(lowerNote)) { targetList = 't3'; tagFound = 'T3'; description = 'Cancellation request'; }
      else if (/\[?t(ier\s*)?2[^\]]*\]?/i.test(lowerNote)) { targetList = 't2'; tagFound = 'T2'; description = 'Client is very upset'; }
      else if (/\[?t(ier\s*)?1[^\]]*\]?/i.test(lowerNote)) { targetList = 't1'; tagFound = 'T1'; description = 'Client needs reassurance'; }
      else if (/\[?t(ier\s*)?0[^\]]*\]?/i.test(lowerNote)) { targetList = 't0'; tagFound = 'T0'; description = 'Client not upset'; }
    }
