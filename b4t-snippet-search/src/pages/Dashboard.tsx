...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch('/api/data/report')
      .then(res => res.json())
      .then(data => {
        const sorted = (Array.isArray(data) ? data : []).sort((a, b) => b.TotalBalance - a.TotalBalance);
                    ...
                    <div className="border border-gray-200 rounded-xl p-6 bg-white shadow-sm mt-4">
                      <div className="flex justify-between items-end mb-3">
                        ...
                        <div className="text-[11px] text-gray-400 font-bold uppercase tracking-widest">
                          Total Invoiced Volume: ${(selectedClient.TotalAmountPaid + selectedClient.OutstandingBalance).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                        </div>
                      </div>
                      ...
                      <div className="h-4 bg-gray-100 rounded-full overflow-hidden w-full relative">
                        <div 
                          ...
                          style={{width: `${Math.min(100, Math.max(5, (selectedClient.TotalAmountPaid / (selectedClient.TotalAmountPaid + selectedClient.OutstandingBalance || 1)) * 100))}%`}} 
                        />
                      </div>
                      
                      <div className="flex justify-between items-center mt-3">
                        <div className="text-xs text-gray-500 font-medium">
                          {Math.round((selectedClient.TotalAmountPaid / (selectedClient.TotalAmountPaid + selectedClient.OutstandingBalance || 1)) * 100)}% Paid to date
                        </div>
                        <div className="text-[11px] font-bold text-gray-700">
                          {selectedClient.OutstandingBalance > 0 ? 'Partial Payment' : 'Fully Paid'}
                        </div>
                      </div>
                    </div>
                  </div>
